...
function loadCsv(filename: string): Array<Record<string, string>> {
...
export function getDataStore(): DataStore {
  ...
  const districts: District[] = loadCsv('districts.csv').map((r) => ({
    district: r.district,
  ...
  const parcels: Parcel[] = loadCsv('sample_parcels.csv').map((r) => ({
  ...
  const investors: Investor[] = loadCsv('sample_investors.csv').map((r) => ({
  ...
  const transactions: Transaction[] = loadCsv('sample_transactions.csv').map((r) => ({
  ...
  const communities: Community[] = loadCsv('sample_communities.csv').map((r) => ({
